...
async function close(
  ownerSigner: web3.Keypair,
  programId: web3.PublicKey,
  connection: web3.Connection
) {
  let closeBuffer = Buffer.alloc(1000);
  ...
  const [pda] = await web3.PublicKey.findProgramAddress(
    [ownerSigner.publicKey.toBuffer(), Buffer.from("config-prize")],
    programId
  );
  ...
  closeBuffer = closeBuffer.slice(
    0,
    configInstructionLayout.getSpan(closeBuffer)
  );
...
async function main() {
  const ownerSigner = initializeSignerKeypair();
  ...
  await close(ownerSigner, prizeProgramId, connection);
  await getConfig(PDA, connection);
}
